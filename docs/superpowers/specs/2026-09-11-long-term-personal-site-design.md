...
- 页面底部的版权与内容授权标识统一署名 `Jingtine`，不使用中文名或站点品牌替代作者名
...
现有纸张纹理、浅紫蓝配色、衬线标题、侧栏结构和宽正文版式继续作为基础。升级重点是减少规则卡片、粗边框、连续编号与英文技术标签的密度，让内容呈现更像一份由个人长期编辑的刊物。

- 首页从技术履历目录改为“主人近况、最近随笔、正在做的事、随手收藏”的阅读顺序。
- 首页主标题使用“你好，我是不驚醴。”，软件工程、AI Agent 与产品创新降为个人经历的一部分，不再定义整站主题。
- 章节编号只保留在少数需要刊物感的标题处，不再从 `01` 连续编号到页面末尾。
- 大面积有框卡片减少约一半，优先使用留白、细分隔线、错落图文和纸张层次。
- 胶带、手写批注与彩色标记作为低频强调，不在每个模块重复使用。
- 随笔封面成为主要视觉；无封面内容依靠标题排版、分类色和轻量图形成立。
- 浅紫蓝仍是基础色，茶褐、墨色和褪红只用于小面积批注与状态提示。
- 移动端避免连续堆叠高卡片，压缩装饰高度并保持正文优先。
